 ...
 */
const fetchTicketsByOrderFromDB = ({
  pageParam,
  orderBy,
}: {
  pageParam: number;
  ...
  return db.tickets.orderBy(orderBy).offset(offset).limit(BATCH_SIZE).toArray();
};
...
export const useAllTickets = () =>
  useQueries({
    ...
    combine: (results) => {
      return {
        data: results.map((result) => result.data?.tickets),
...
export const useTicketsByOrder = (
  ...
  useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) =>
      fetchTicketsByOrderFromDB({ pageParam, orderBy }),
    initialPageParam: 0,
